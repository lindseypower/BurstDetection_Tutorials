...
def spectralevents_vis ( specEv, timeseries, TFR, tVec, fVec, eventBand ):
    ...
    ########################
    # Plot TFR data
    fig, axs = plt.subplots(nrows=numSampTrials+1, ncols=1)

    # Plot average TFR
    im = axs[0].pcolor(tVec, fVec, avgTFR, cmap='jet')
    fig.colorbar(im, ax=axs[0])
    axs[0].invert_yaxis()
    axs[0].axhline(y=eventBand[0])
    axs[0].axhline(y=eventBand[1])
    ...
    for threshold in thresholds:
        ...
        for spectrogram in trial_tfrs:	
            ...
            for i in np.arange(0,16):
                freq_spectrogram_thresholded = np.where(spectrogram[i] > median[i]*threshold, 1, 0)
